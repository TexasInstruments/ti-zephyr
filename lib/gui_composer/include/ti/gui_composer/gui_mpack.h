...
#ifndef ZEPHYR_INCLUDE_TI_GUI_COMPOSER_GUI_MPACK_H_
#define ZEPHYR_INCLUDE_TI_GUI_COMPOSER_GUI_MPACK_H_
#include <stdbool.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
typedef void (*gui_rx_callback_cmd_t)(mpack_tag_t *, mpack_reader_t *);
 ...
 * @param count Number of bytes in str to send.
 */
extern void gui_transmit_string_blocking(char *str, size_t count);

/**
 * @brief Recovers the shared mpack writer after an encoding error.
 *
 * mpack latches its writer into a permanent error state on the first
 * failure (e.g. a message too large for the TX buffer): every
 * subsequent write silently no-ops until the writer is reinitialized.
 * Called by guicomm_mpack.c's GUIComm_send* functions when they detect
 * writer.error != mpack_ok, so a single transient encoding failure
 * does not permanently silence all further GUI Composer output.
 */
extern void gui_reset_writer(void);

#endif /* ZEPHYR_INCLUDE_TI_GUI_COMPOSER_GUI_MPACK_H_ */
