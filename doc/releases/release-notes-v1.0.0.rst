...
* ``zephyr/module.yml`` — registers ti-zephyr as a Zephyr module
  (``board_root``, ``dts_root``, ``snippet_root``, ``kconfig``, ``cmake``).
* ``Kconfig`` / ``CMakeLists.txt`` — module-level build entry points that
  wire ``drivers/`` into the Zephyr Kconfig/CMake tree.
