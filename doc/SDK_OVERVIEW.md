...
│
├── Kconfig                          ← module Kconfig entry point
├── CMakeLists.txt                   ← module CMake entry point
│
├── zephyr/
│   └── module.yml                  ← registers as Zephyr module
│                                      (board_root, dts_root, snippet_root,
│                                       kconfig: Kconfig, cmake: .)
│
...
└── snippets/                       ← downstream build snippets (empty, staged)
```
...
| `Kconfig` / `CMakeLists.txt` | Module-level build entry points (rsource/add_subdirectory into `drivers/`) |
