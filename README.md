...
├── Kconfig          ← module Kconfig entry point (rsources drivers/Kconfig)
├── CMakeLists.txt   ← module CMake entry point (add_subdirectory(drivers))
├── zephyr/
│   └── module.yml   ← registers as Zephyr module (board_root, dts_root,
│                        kconfig, cmake — see zephyr-silabs for the
│                        equivalent pattern)
├── boards/ti/       ← TI boards not yet in upstream
├── drivers/         ← downstream TI drivers
├── dts/             ← downstream TI DTS/DTSI
├── snippets/        ← downstream build snippets
